...
16. **Alcune semantiche pubbliche sono ambigue.**
    `SetWorkAntenna` e `GetWorkAntenna` ora usano entrambi indici zero-based; `Tag::antenna_index` espone lo stesso criterio per le letture RFID. `Spectrum::CUSTOM` ora restituisce `UnsupportedResponse` invece di frequenze fittizie. Restano da sistemare le frequenze `f64` confrontate per uguaglianza esatta. Il `Display` errato di `Reset` è stato corretto nella Fase 0.
...
- [x] Decidere il comportamento reale di `Spectrum::CUSTOM`; fino all'implementazione, restituire `UnsupportedResponse` anziché dati fittizi.
